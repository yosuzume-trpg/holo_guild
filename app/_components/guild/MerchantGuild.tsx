...
export default function MerchantGuild() {
  ...
  for (const char of assignedMerchants) {
    ...
    ratePerItem[asgn.sellMaterialId] =
      (ratePerItem[asgn.sellMaterialId] ?? 0) + getMerchantRate(char.merchantLevel, researchBonus)
  }
    ...
    <div className="p-4 space-y-4">
      ...
      <AssignedSlotList
        ...
        slotProgress={(char) => {
          ...
          return getAutoProgress(frac[`merchant:${char.id}`] ?? 0, rate, lastTick, Date.now())
        }}
        renderInfo={(char) => {
          const asgn = char.assignment as Extract<typeof char.assignment, { type: 'merchant' }>
          ...
          return (
            <>
              ...
              {item && (
                <>
                  <div className=" text-success">{item.name} ({rate.toFixed(2)}/分)</div>
                  <div className="text-ink-subtle">最低{asgn.minStock}個 ・ 在庫{stock}</div>
                </>
              )}
            </>
          )
